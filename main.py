...
from src import Config, load_model, CleanData, inference, process_data
...
class InferenceData(BaseModel):
    ...
    fnlgt: int = Field(..., example=94638)
    education: str = Field(..., example="HS-grad")
    education_num: int = Field(..., alias='education-num', example=9)
    marital_status: str = Field(..., alias='marital-status', example="Separated")
...
def predict(data: InferenceData):
    ...
    data_processed, _, _, _ = process_data(X=df,
